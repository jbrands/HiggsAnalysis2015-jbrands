...
void run_mktree(TString particle="mu", TString in="", TString out=""){//float splitFactor_sig=0.5, float splitFactor_bg=splitFactor_sig){
  ...
  if (particle=="ele"){
    ...
    if (in=="") in="/data/jbrandstetter/CMGTools/rootFiles_151211/additionalSelection/test_mt/";
    if (out=="") out="/data/jbrandstetter/CMGTools/rootFiles_151211/additionalSelection/test_mt/BDTinput/";
  }
  else {
  ...
  */
  if (particle=="mu"){
    fname.push_back(dir+"Ntuple_basis_isoCuts_MC_VBFHiggs_mt.root"); signal.push_back(1);
    fname.push_back(dir+"Ntuple_basis_isoCuts_MC_WJetsToLNu_madgraphMLM_mt.root"); signal.push_back(0);
    fname.push_back(dir+"Ntuple_basis_isoCuts_MC_DYJetsToLL_madgraphMLM_mt.root"); signal.push_back(0);
    fname.push_back(dir+"Ntuple_basis_isoCuts_MC_TTbar_powheg_mt.root"); signal.push_back(0);
